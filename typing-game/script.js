...
/*script.js - 초기값 세팅
words: 현재 선택된 인용문을 단어 단위로 분할하여 배열로 저장
wordIndex: 현재 타이핑해야 할 단어의 인덱스를 추적
startTime: 타이핑 시작 시간을 기록하여 총 소요 시간을 계산*/
let words = [];
let wordIndex = 0;
...
document.getElementById('start').addEventListener('click',() => {
    ...
    typedValueElement.disabled = false; // 게임 완료 시 입력 필드 비활성화

   });

//이벤트 리스너 설정 - input 필드의 입력 이벤트
typedValueElement.addEventListener('input', () => {
    const currentWord = words[wordIndex]; // 현재 타이핑할 단어를 currentWord에 저장
    const typedValue = typedValueElement.value; // 입력한 값을 typeValue에 저장
    ...
    typedValueElement.disabled = true; // 게임 완료 시 입력 필드 비활성화
    ...
    typedValueElement.value = ''; // 입력 필드를 초기화하여 다음 단어 입력 준비
    wordIndex++; // 다음 단어로 이동
